...
def build_dataset(cfg, mode='train', is_source=True, epochwise=False):
    ...
    if mode=='train':
        ...
        if is_source:
            dataset = DatasetCatalog.get(cfg.DATASETS.SOURCE_TRAIN, mode, num_classes=cfg.MODEL.NUM_CLASSES, max_iters=iters, transform=transform, ignore_label=cfg.INPUT.IGNORE_LABEL, data_dir = cfg.data_dir, distill_path = cfg.distill_path)
        else:
            dataset = DatasetCatalog.get(cfg.DATASETS.TARGET_TRAIN, mode, num_classes=cfg.MODEL.NUM_CLASSES, max_iters=iters, transform=transform, ignore_label=cfg.INPUT.IGNORE_LABEL, data_dir = cfg.data_dir, distill_path = cfg.distill_path)
    elif mode=='val':
        dataset = DatasetCatalog.get(cfg.DATASETS.TEST, 'val', num_classes=cfg.MODEL.NUM_CLASSES, max_iters=iters, transform=transform, ignore_label=cfg.INPUT.IGNORE_LABEL, data_dir = cfg.data_dir, distill_path = cfg.distill_path)
    elif mode=='test':
        dataset = DatasetCatalog.get(cfg.DATASETS.TEST, cfg.DATASETS.TEST.split('_')[-1], num_classes=cfg.MODEL.NUM_CLASSES, max_iters=iters, transform=transform, ignore_label=cfg.INPUT.IGNORE_LABEL, data_dir = cfg.data_dir, distill_path = cfg.distill_path)
    return dataset
